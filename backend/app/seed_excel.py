...
def run_excel_seed(file_path: str):
    ...
    # We also need the profiles sheet to map the original generated UUID to the new Auth UUID
    ...
    for _, row in auth_df.iterrows():
        ...
        try:
            ...
            existing = sb.table('profiles').select('id').eq('email', email).maybe_single().execute()
            if existing.data:
                print(f"User {email} already exists in DB. Re-using ID.")
                uuid_map[old_uuid] = existing.data['id']
                continue
    ...
    import_sequence = [
        ...
        ('daily_stats', ['worker_profile_id']), # table name: platform_worker_daily_stats
        ('order_events', ['worker_profile_id', 'pickup_zone_id', 'drop_zone_id']), # table name: platform_order_events
        ('trigger_events', ['zone_id']),
        ('manual_claims', ['worker_profile_id', 'trigger_event_id', 'shift_id']),
    ...
    for sheet_name, fk_cols in import_sequence:
        ...
        for _, row in df.iterrows():
            ...
            if sheet_name == 'payout_recommendations' and 'explanation_json' in row_dict:
                import json
                if isinstance(row_dict['explanation_json'], str):
                    try:
                        row_dict['explanation_json'] = json.loads(row_dict['explanation_json'])
                    except:
                        pass
        ...
        print(f"Importing {len(records_to_insert)} records into '{table_name}'...")
        for i in range(0, len(records_to_insert), chunk_size):
            chunk = records_to_insert[i:i + chunk_size]
            try:
                # Upsert to handle re-runs gracefully (assuming 'id' or 'ref_id' is present)
