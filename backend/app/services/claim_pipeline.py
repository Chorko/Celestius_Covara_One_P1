...
def run_claim_pipeline(
    ...
    if claim_mode == "manual":
        ...
        if mv["manual_verification_status"] == "hold":
            ...
            add_trace(3, "manual_strictness", f"Held: {hold_reason_text}")
        else:
            add_trace(
                3,
                "manual_strictness",
    ...
    add_trace(
        ...
        (
            f"fraud_score={fraud_res['fraud_score']} "
            f"band={fraud_res['fraud_band']} "
            f"decision={fraud_res['recommended_action']} "
            f"flags={fraud_res['flag_count']}"
        ),
    )
    ...
    final_status = STATUS_MAP.get(
        decision_status, "soft_hold_verification"
    )
