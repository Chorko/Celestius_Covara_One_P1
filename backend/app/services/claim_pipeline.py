...
def run_claim_pipeline(
    ...
    if claim_mode == "manual":
        ...
        if mv["manual_verification_status"] == "hold":
            ...
            reasons_str = ", ".join(hold_reasons)
            add_trace(3, "manual_strictness", f"Held: {reasons_str}")
        else:
            add_trace(
                3,
                "manual_strictness",
    ...
    add_trace(6, "anti_spoofing_fraud_check", f"fraud_score={fraud_score} band={fraud_band} decision={fraud_action} flags={fraud_flags}")
