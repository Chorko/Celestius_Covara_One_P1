...
def evaluate_manual_claim(claim_record: dict, evidence_records: list[dict], worker_context: dict) -> dict:
    """
    ...
    exif_geo_exists = any(e.get("exif_lat") is not None for e in evidence_records)
    
    if stated_geo_exists or exif_geo_exists:
        compl_score += 0.3
        
    if compl_score < 0.7:
        hold_reasons.append(f"Incomplete evidence (score: {compl_score}). Need at least photo/video and location.")
    ...
    if not stated_geo_exists and not exif_geo_exists:
        geo_score -= 0.5
        hold_reasons.append("No geographic location provided with claim or evidence.")
    ...
    if zone_lat and zone_lng:
        ...
        if test_lat and test_lng:
            zone_res = verify_zone_match(test_lat, test_lng, zone_lat, zone_lng)
            if zone_res["match_level"] == "outside_zone":
                geo_score = 0.0
                hold_reasons.append(f"Location is too far ({zone_res['distance_km']}km) from assigned zone center.")
            elif zone_res["match_level"] == "near_zone_boundary":
                geo_score -= 0.2
                hold_reasons.append(f"Location is at edge of zone ({zone_res['distance_km']}km).")
    ...
    for e in evidence_records:
        ...
        if exif_ts:
            ...
            try:
                ...
                if delta_days > 3:
                    hold_reasons.append(f"Photo EXIF timestamp is {delta_days} days old.")
    ...
    review_status = "hold" if len(hold_reasons) > 0 else "proceed_to_fraud_check"
