...
# In production, this would be a maintained database (e.g., ip2proxy, ipinfo.io)
DATACENTER_CIDRS = [
...
EMULATOR_MARKERS = [
    "BlueStacks", "Nox", "Genymotion", "MEmu", "LDPlayer",
    "google_sdk", "generic_x86", "vbox86p", "Andy", "Xamarin"
]

# ── EXIF vs Browser GPS Cross-Check ─────────────────────────────────────

def check_exif_gps_mismatch(evidence_records: list[dict], claim_lat: float, claim_lng: float) -> dict:
    """
    ...
    for ev in evidence_records:
        ...
        if exif_lat is not None and exif_lng is not None and claim_lat and claim_lng:
            dist_km = calculate_distance_km(exif_lat, exif_lng, claim_lat, claim_lng)
            dist_m = dist_km * 1000
    ...
    return {
        ...
        "exif_browser_gps_distance_m": round(best_distance_m, 1) if best_distance_m is not None else None,
...
def check_exif_timestamp_freshness(evidence_records: list[dict], claim_timestamp: str = None) -> dict:
    """
    ...
    if claim_timestamp:
        try:
            claim_dt = datetime.fromisoformat(claim_timestamp.replace("Z", "+00:00")).replace(tzinfo=None)
        except Exception:
            claim_dt = now
    ...
    for ev in evidence_records:
        ...
        try:
            ...
            if lag_hours <= 2:
                ...
                stale_evidence.append({"evidence_id": ev.get("id"), "lag_hours": round(lag_hours, 1)})
            else:
...
def check_vpn_datacenter_ip(claim_ip: str) -> dict:
    ...
    """
    if not claim_ip:
        return {"vpn_flag": False, "reason": "no_ip_provided", "risk_level": "unknown"}
    ...
    try:
        ...
        return {"vpn_flag": False, "reason": "invalid_ip", "risk_level": "unknown"}
...
def check_device_continuity(worker_context: dict, claim_device_id: str = None) -> dict:
    """
    ...
    if not claim_device_id:
        return {"device_match": None, "new_device": False, "risk_level": "uncertain"}

    is_match = (claim_device_id == registered_device) if registered_device else False
    is_new = (claim_device_id not in device_history) if device_history else True
    ...
    return {
        ...
        "new_device": is_new,
        "device_count_on_account": len(set(device_history)) if device_history else 0,
...
def check_movement_plausibility(
    ...
    if not all([last_lat, last_lng, last_ts, claim_lat, claim_lng]):
        return {"impossible_travel": False, "reason": "insufficient_data", "risk_level": "uncertain"}
    ...
    try:
        ...
        distance_km = calculate_distance_km(last_lat, last_lng, claim_lat, claim_lng)
        ...
        return {"impossible_travel": False, "reason": "calculation_error", "risk_level": "uncertain"}
...
def check_emulator_signals(device_context: dict) -> dict:
    ...
    return {
        ...
        "risk_level": "critical" if len(flags) >= 2 else ("elevated" if len(flags) == 1 else "low")
    }
...
def verify_anti_spoofing(
    ...
    claim_ts = claim_data.get("created_at") or claim_data.get("claim_timestamp")
    claim_ip = claim_data.get("client_ip")
    claim_device_id = claim_data.get("device_id")

    # Run all checks
    gps_check = check_exif_gps_mismatch(evidence_records, claim_lat, claim_lng)
    freshness_check = check_exif_timestamp_freshness(evidence_records, claim_ts)
    vpn_check = check_vpn_datacenter_ip(claim_ip)
    device_check = check_device_continuity(worker_context, claim_device_id)
    movement_check = check_movement_plausibility(worker_context, claim_lat, claim_lng, claim_ts)
    ...
    score_components.append(("evidence_freshness", freshness_check["freshness_score"], 0.15))
    ...
    score_components.append(("device_continuity", dev_scores.get(device_check["risk_level"], 0.5), 0.15))
    ...
    score_components.append(("emulator_check", emu_scores.get(emulator_check["risk_level"], 0.5), 0.15))
    ...
    return {
        ...
        "requires_liveness_check": device_check.get("requires_liveness", False),
        "checks": {
        ...
        "score_breakdown": {name: round(score, 4) for name, score, _ in score_components}
    }
